...
function f_main_community_reply_add(communitySeq, depth, moReplySeq, writer, el){
    if(nvl(writer,'') !== ''){
        ...
        if(nvl(content,'') === ''){ showMessage('', 'error', '[댓글 정보]', '댓글 내용을 입력해 주세요.', ''); return false; }
        if(!write_slang_check(content)){ showMessage('', 'error', '[댓글 정보]', '댓글에 사용할 수 없는 단어가 포함되어 있습니다.<br>(욕설, 음란성 단어 등)', ''); return false; }
        if(depth === '1'){
        ...
        $.ajax({
            url: '/job/community/reply/check.do',
            method: 'POST',
            async: false,
            data: JSON.stringify(jsonObj),
            dataType: 'json',
            ...
            success: function (data) {
                if (data.resultCode === "0") {
                    if (Number.parseInt(data.customValue) < 20) {
                        ...
                        showMessage('', 'info', '[커뮤니티]', '일일 등록 가능한 댓글 수는 최대 20개입니다.', '');
                    }
                }
            }
        });
...
function f_main_community_add(id){
    ...
    if(!write_slang_check(title)){ showMessage('', 'error', '[게시글 정보]', '제목에 사용할 수 없는 단어가 포함되어 있습니다.<br>(욕설, 음란성 단어 등)', ''); return false; }

    let content = $('#quill_content').val().replaceAll(/<[^>]*>?/g, '');
    if(nvl(content,'') === ''){ showMessage('', 'error', '[게시글 정보]', '내용을 입력해 주세요.', ''); return false; }
    if(!write_slang_check(content)){ showMessage('', 'error', '[게시글 정보]', '내용에 사용할 수 없는 단어가 포함되어 있습니다.<br>(욕설, 음란성 단어 등)', ''); return false; }
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '/job/community/write/check.do',
                method: 'POST',
                async: false,
                ...
                success: function (data) {
                    if (data.resultCode === "0") {
                        if(Number.parseInt(data.customValue) < 3){
                            $.ajax({
                            ...
                            showMessage('', 'info', '[커뮤니티]', '일일 등록 가능한 게시물 수는 최대 3개입니다.', '');
                        }
                    }else {
                        showMessage('', 'error', '에러 발생', '커뮤니티 글 등록을 실패하였습니다. 관리자에게 문의해주세요. ' + data.resultMessage, '');
                    }
