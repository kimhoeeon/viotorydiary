/***
 * mng/customer/sailyacht
 * 회원/신청>신청자목록>세일요트 기초정비실습 과정
 * */
...
function f_customer_sailyacht_search(){
    ...
    let resData = ajaxConnect('/mng/customer/sailyacht/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_sailyacht_table tbody td');
    let cnt = 0;
...
function f_customer_sailyacht_search_condition_init(){
...
function f_customer_sailyacht_detail_modal_set(seq){
    /* TM 및 잠재DB 목록 상세 조회 */
    let jsonObj = {
        seq: seq
    };
...
function f_customer_sailyacht_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        targetSeq: seq,
                        targetTable: 'sailyacht',
                        deleteReason: result.value,
                        targetMenu: getTargetMenu('mng_customer_sailyacht_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_sailyacht_search(); // 재조회
...
function f_customer_sailyacht_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            /* form valid check */
            let validCheck = f_customer_sailyacht_valid();

            if(validCheck){

                /* form data setting */
                let data = f_customer_sailyacht_form_data_setting();
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_customer_sailyacht_modify_init_set(seq); // 재조회
                                    }
                                });
                            } else {
...
function f_customer_sailyacht_form_data_setting(){
...
function f_customer_sailyacht_valid(){
...
function f_customer_sailyacht_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/sailyacht/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_sailyacht_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '세일요트 기초정비실습 과정' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_sailyacht_train_change_btn(){
