...
function f_customer_emergency_search(){
    ...
    let resData = ajaxConnect('/mng/customer/emergency/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_emergency_table tbody td');
    let cnt = 0;
...
function f_customer_emergency_search_condition_init(){
...
function f_customer_emergency_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        targetSeq: seq,
                        targetTable: 'emergency',
                        deleteReason: result.value,
                        targetMenu: getTargetMenu('mng_customer_emergency_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_emergency_search(); // 재조회
...
function f_customer_emergency_modify_init_set(seq){
    window.location.href = '/mng/customer/emergency/detail.do?seq=' + seq;
}

function f_customer_emergency_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            /* form valid check */
            let validCheck = f_customer_emergency_valid();

            if(validCheck){

                /* form data setting */
                let data = f_customer_emergency_form_data_setting();
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_customer_emergency_modify_init_set(seq); // 재조회
                                    }
                                });
                            } else {
...
function f_customer_emergency_form_data_setting(){
...
function f_customer_emergency_valid(){
        ...
        if(nvl(md_cancel_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/emergency/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/emergency/status/change/update.do', 'post', jsonArr);
...
function f_customer_emergency_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/emergency/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_emergency_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '응급조치교육' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_emergency_train_change_btn(){
