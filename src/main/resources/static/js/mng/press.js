...
function f_board_press_search(){
    ...
    let resData = ajaxConnect('/mng/board/press/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_board_press_table tbody td');
    let cnt = 0;
...
function f_board_press_search_condition_init(){
...
function f_board_press_detail_modal_set(seq){
    /* TM 및 잠재DB 목록 상세 조회 */
    let jsonObj = {
        seq: seq
    };

    let resData = ajaxConnect('/mng/board/press/selectSingle.do', 'post', jsonObj);

    /* 보도자료 상세보기 Modal form Set */
    //console.log(resData);
...
function f_board_press_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: "[삭제 사유]",
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        ...
                        targetMenu: getTargetMenu('mng_board_press_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_board_press_search(); // 재조회
...
function f_board_press_modify_init_set(seq){
    window.location.href = '/mng/board/press/detail.do?seq=' + seq;
}

function f_board_press_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if(validCheck){
                ...
                /* form data setting */
                let data = f_board_press_form_data_setting();

                /* Modify */
                if(nvl(seq, '') !== ''){
                    $.ajax({
                        url: '/mng/board/press/update.do',
                        method: 'POST',
                        async: false,
                        data: data,
                        dataType: 'json',
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_board_press_modify_init_set(seq); // 재조회
                                    }
                                });
                            } else {
...
function f_board_press_form_data_setting(){
