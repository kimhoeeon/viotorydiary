/***
 * mng/customer/highspecial
 * 회원/신청>신청자목록>고마력 선외기 정비 중급 테크니션 (특별반)
 * */
...
function f_customer_highspecial_search(){
    ...
    let resData = ajaxConnect('/mng/customer/highspecial/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_highspecial_table tbody td');
    let cnt = 0;
...
function f_customer_highspecial_search_condition_init(){
...
function f_customer_highspecial_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        targetSeq: seq,
                        targetTable: 'highspecial',
                        deleteReason: result.value,
                        targetMenu: getTargetMenu('mng_customer_highspecial_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_highspecial_search(); // 재조회
...
function f_customer_highspecial_modify_init_set(seq){
    window.location.href = '/mng/customer/highspecial/detail.do?seq=' + seq;
}
        ...
        if(nvl(md_cancel_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/highspecial/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/highspecial/status/change/update.do', 'post', jsonArr);
...
function f_customer_highspecial_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/highspecial/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_highspecial_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '고마력 선외기 정비 중급 테크니션 (특별반)' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_highspecial_train_change_btn(){
