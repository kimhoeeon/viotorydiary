/***
 * mng/customer/boarder
 * 회원/신청>신청자목록>해상엔진 테크니션 (선내기/선외기)
 * */
...
function f_customer_boarder_search(){
    ...
    let jb = $('#mng_customer_boarder_table tbody td');
    let cnt = 0;
...
function f_customer_boarder_detail_modal_set(seq){
    /* TM 및 잠재DB 목록 상세 조회 */
    let jsonObj = {
        seq: seq
    };
...
function f_customer_boarder_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: "[삭제 사유]",
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        ...
                        targetMenu: getTargetMenu('mng_customer_boarder_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_boarder_search(); // 재조회
...
function f_customer_boarder_modify_init_set(seq){
    window.location.href = '/mng/customer/boarder/detail.do?seq=' + seq;
}

function f_customer_boarder_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            /* form valid check */
            let validCheck = f_customer_boarder_valid();

            if(validCheck){

                /* form data setting */
                let data = f_customer_boarder_form_data_setting();
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_customer_boarder_modify_init_set(seq); // 재조회
                                    }
                                });
                            } else {
...
function f_customer_boarder_form_data_setting(){
        ...
        if(nvl(md_cancel_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/boarder/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/boarder/status/change/update.do', 'post', jsonArr);
...
function f_customer_boarder_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/boarder/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_boarder_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '해상엔진 테크니션 (선내기/선외기)' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_boarder_train_change_btn(){
