/***
 * mng/customer/competency
 * 회원/신청>신청자목록>선외기/선내기 직무역량 강화과정
 * */
...
function f_customer_competency_search(){
    ...
    let resData = ajaxConnect('/mng/customer/competency/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_competency_table tbody td');
    let cnt = 0;
...
function f_customer_competency_search_condition_init(){
...
function f_customer_competency_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        ...
                        targetMenu: getTargetMenu('mng_customer_competency_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_competency_search(); // 재조회
...
function f_customer_competency_modify_init_set(seq){
    window.location.href = '/mng/customer/competency/detail.do?seq=' + seq;
}
        ...
        if(nvl(md_cancel_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/competency/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/competency/status/change/update.do', 'post', jsonArr);
...
function f_customer_competency_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/competency/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_competency_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '선외기/선내기 직무역량 강화과정' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_competency_train_change_btn(){
