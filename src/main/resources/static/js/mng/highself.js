/***
 * mng/customer/highself
 * 회원/신청>신청자목록>자가정비 심화과정 (고마력 선외기)
 * */
...
function f_customer_highself_search(){
    ...
    let resData = ajaxConnect('/mng/customer/highself/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_highself_table tbody td');
    let cnt = 0;
...
function f_customer_highself_search_condition_init(){
...
function f_customer_highself_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        targetSeq: seq,
                        targetTable: 'highself',
                        deleteReason: result.value,
                        targetMenu: getTargetMenu('mng_customer_highself_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_highself_search(); // 재조회
...
function f_customer_highself_modify_init_set(seq){
    window.location.href = '/mng/customer/highself/detail.do?seq=' + seq;
}
        ...
        if(nvl(md_cancel_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/highself/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/highself/status/change/update.do', 'post', jsonArr);
...
function f_customer_highself_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/highself/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_highself_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '자가정비 심화과정 (고마력 선외기)' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_highself_train_change_btn(){
