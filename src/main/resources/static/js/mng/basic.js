...
function f_customer_basic_search(){
    ...
    let resData = ajaxConnect('/mng/customer/basic/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_basic_table tbody td');
    let cnt = 0;
...
function f_customer_basic_search_condition_init(){
...
function f_customer_basic_detail_modal_set(seq){
    /* TM 및 잠재DB 목록 상세 조회 */
    let jsonObj = {
        seq: seq
    };
...
function f_customer_basic_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    let jsonObj = {
                        ...
                        targetMenu: getTargetMenu('mng_customer_basic_table'),
                        delYn: 'Y'
                    }
                    f_mng_trash_remove(jsonObj);

                    f_customer_basic_search(); // 재조회
...
function f_customer_basic_modify_init_set(seq){
    window.location.href = '/mng/customer/basic/detail.do?seq=' + seq;
}

function f_customer_basic_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            /* form valid check */
            let validCheck = f_customer_basic_valid();

            if(validCheck){

                /* form data setting */
                let data = f_customer_basic_form_data_setting();
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        f_customer_basic_modify_init_set(seq); // 재조회
                                    }
                                });
                            } else {
...
function f_customer_basic_form_data_setting(){
...
function f_customer_basic_valid(){
        ...
        if(nvl(md_cancel_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/basic/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/basic/status/change/update.do', 'post', jsonArr);
...
function f_customer_basic_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/basic/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_basic_train_change_modal_set(seq, name, applyStatus, nextTime){

    if(!applyStatus.includes('취소')){
        // 교육 Set
        $("#md_edu").children('option:not(:gt(1))').remove();

        let jsonObj = { gbn : '기초정비교육' };
        $.ajax({
            url: '/train/active.do',
            method: 'post',
            data: JSON.stringify(jsonObj),
...
function f_customer_basic_train_change_btn(){
