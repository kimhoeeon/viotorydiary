...
function f_customer_famtourin_search(){
    ...
    let resData = ajaxConnect('/mng/customer/famtourin/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_customer_famtourin_table tbody td');
    let cnt = 0;
...
function f_customer_famtourin_search_condition_init(){
...
function f_customer_famtourin_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        Swal.fire({
            title: '[ 삭제 사유 ]',
            ...
            if (result.isConfirmed) {
                if (result.value) {
                    ...
                    f_customer_famtourin_search(); // 재조회
...
function f_customer_famtourin_modify_init_set(seq){
    window.location.href = '/mng/customer/famtourin/detail.do?seq=' + seq;
}
    ...
    if (nvl(idArr,'') !== ''){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let resData = ajaxConnect('/mng/customer/famtourin/status/update.do', 'post', jsonArr);
        ...
        if(nvl(md_status_gbn_val,'') !== '') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let resData = ajaxConnect('/mng/customer/famtourin/status/change/update.do', 'post', jsonArr);
...
function f_customer_famtourin_detail_excel_download(tableId , name){
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.resultCode === '0') {
                                ...
                                $.ajax({
                                    url: '/mng/customer/famtourin/excel/download.do?fileName=' + fileName,
                                    method: 'get',
                                    /*async: false,*/
                                    xhrFields: {
                                        responseType: 'blob'
...
function f_customer_famtourin_train_change_modal_set(seq, name, applyStatus, nextTime){
...
function f_customer_famtourin_train_change_btn(){
