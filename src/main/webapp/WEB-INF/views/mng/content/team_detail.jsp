...
<head>
    <meta charset="utf-8">
    <title>콘텐츠 상세 | Viotory Admin</title>
                                    ...
                                    <div class="card-body p-9">
                                        <div class="row mb-7">
                                            ...
                                            <div class="col-lg-4"><span
                                                    class="badge badge-light-primary fw-bold fs-6">${content.teamCode}</span>
                                            </div>
                                            <label class="col-lg-2 fw-semibold text-muted">정렬 순서</label>
                                            <div class="col-lg-4"><span
                                                    class="fw-bold fs-6 text-gray-800">${content.sortOrder}</span></div>
                                        </div>
                                        <div class="row mb-7">
                                            <label class="col-lg-2 fw-semibold text-muted">제목</label>
                                            <div class="col-lg-10"><span
                                                    class="fw-bold fs-6 text-gray-800">${content.title}</span></div>
                                        </div>
                                        <div class="row mb-7">
                                            <label class="col-lg-2 fw-semibold text-muted">URL</label>
                                            <div class="col-lg-10"><a href="${content.contentUrl}" target="_blank"
                                                                      class="text-primary fw-semibold fs-6">${content.contentUrl}</a>
                                            </div>
                                        </div>
                                        <div class="row mb-7">
                                            <label class="col-lg-2 fw-semibold text-muted">상태</label>
                                            <div class="col-lg-4">
                                                <c:if test="${content.status eq 'ACTIVE'}"><span
                                                        class="badge badge-light-success">노출</span></c:if>
                                                <c:if test="${content.status eq 'INACTIVE'}"><span
                                                        class="badge badge-light-secondary">숨김</span></c:if>
                                            </div>
                                            <label class="col-lg-2 fw-semibold text-muted">조회수</label>
                                            <div class="col-lg-4"><span
                                                    class="fw-bold fs-6 text-gray-800">${content.clickCount}</span></div>
                                        </div>
                                    </div>
                        ...
                        <div class="row mb-7">
                            <div class="col-md-8">
                                ...
                                <select class="form-select form-select-solid" name="teamCode">
                                    ...
                                    <option value="SAMSUNG" ${content.teamCode eq 'SAMSUNG' ? 'selected' : ''}>삼성 라이온즈
                                    </option>
                                    <option value="HANWHA" ${content.teamCode eq 'HANWHA' ? 'selected' : ''}>한화 이글스</option>
                                    <option value="KIWOOM" ${content.teamCode eq 'KIWOOM' ? 'selected' : ''}>키움 히어로즈
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="fs-6 fw-semibold mb-2">정렬 순서</label>
                                <input type="number" class="form-control form-control-solid" name="sortOrder"
                                       value="${content.sortOrder}"/>
                            </div>
                        </div>
                        <div class="fv-row mb-7">
                            <label class="required fs-6 fw-semibold mb-2">제목</label>
                            <input type="text" class="form-control form-control-solid" name="title" value="${content.title}"
                                   required/>
                        </div>
                        <div class="fv-row mb-7">
                            <label class="required fs-6 fw-semibold mb-2">콘텐츠 URL</label>
                            <input type="text" class="form-control form-control-solid" name="contentUrl"
                                   value="${content.contentUrl}" required/>
                        </div>
                        <div class="fv-row mb-7">
